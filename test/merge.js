...
describe('Merge', function() {
  ...
  it('{a:[1]} + {b:[2], a:[2]} should retrun {a:[1,2], b:[2]}', function() {
    assert.deepEqual({ a: [1, 2], b: [2] },
      merge({ a: [1] }, { b: [2], a: [2] })
    );
  });
  it('{a:[1], c: "a"} + {b:[2], a:[2]} should retrun {a:[1,2], b:[2], c: "a"}', function() {
    assert.deepEqual({ a: [1, 2], b: [2], c: 'a' },
      merge({ a: [1], c: 'a' }, { b: [2], a: [2] })
    );
  });
  ...
  it('{title: "abc"} + {title: "def"} should retrun {title: ["abc", "def"]}', function() {
    assert.deepEqual({ title: ['abc', 'def'] },
      merge({ title: 'abc' }, { title: 'def' })
    );
  });
  it('{title: "abc"} + {title: "def"} + {title: "xyz"} should retrun {title: ["abc", "def", "xyz"]}', function() {
    assert.deepEqual({ title: ['abc', 'def', 'xyz'] },
      merge(merge({ title: 'abc' }, { title: 'def' }), { title: 'xyz' })
    );
  });
  it('{title: "abc", Author:["Peter", "Paul"]} + {title: "def", Author:["Mary", "May"]} should retrun {title: ["abc", "def"], Author:["Peter", "Paul", "Mary", "May"]}', function() {
    assert.deepEqual({ title: ['abc', 'def'], Author: ['Peter', 'Paul', 'Mary', 'May'] },
      merge({ title: 'abc', Author: ['Peter', 'Paul'] }, { title: 'def', Author: ['Mary', 'May'] })
    );
  });
  it('{Author:["Peter", "Paul"]} + {Author:"Paul"} should retrun {Author:["Peter", "Paul"]}', function() {
    assert.deepEqual({ Author: ['Peter', 'Paul'] },
      merge({ Author: ['Peter', 'Paul'] }, { Author: 'Paul' })
    );
  });
