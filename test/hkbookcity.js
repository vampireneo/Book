...
describe('bookParser', function() {
  ...
  describe('HkBookCity', function() {
    describe('getByISBN()', function() {
      it('should get the correct data of the book "武道狂之詩（卷十四）──山．火．海"', function(
        done
      ) {
        Q.all(eslite.getByISBN('9789881278517')).then(function(value) {
          if (showData) {
            console.log(value);
          }
          ...
          if (
            value.ImageUrl !==
            'http://www.hkbookcity.com/image_book/9789881278517.jpg'
          ) {
            return done('Book cover image does not match.');
          }
          if (!value.hasOwnProperty('Author')) {
            return done('Cannot find Author.');
          }
          if (value.Author !== '喬靖夫') {
            return done('Author does not match - 喬靖夫 != ' + value.Author[0]);
          }
          done();
        });
      });
