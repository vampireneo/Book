      ...
      if (link.length > 0) {
        request(targetUrl, function(error, response, body) {
          if (!error) {
            ...
            bookObj.Title = $(".main_wrap h1[itemprop='name']")
              .text()
              .trim();
            bookObj.SubTitle = $('.main_wrap h2')
              .text()
              .trim();
            bookObj.ImageUrl = $(".main_wrap .cover_img img[itemprop='image']")
              .attr('src')
              .replace(/http:\/\/.*\.book\.com\.tw\/image\/getImage\?i=/, '')
              .replace(/&v=.*/, '');
            bookObj.Author = $(
                ".main_wrap li[itemprop='author'] a[href*='search.books.com.tw']"
              )
              .text()
              .trim()
              .replace(/‧/g, '．');
